...
class upListWindow(QWidget):
    ...
    def updateTable(self):
        ...
        if len(v.sliceQueue) > 0 and v.currentUpLoad != None and v.sliceQueue[0]["path"] != v.currentUpLoad["path"]:
            x += 1
            # if (self.table.rowCount()!=min(len(v.upQueue),v.maxListCount) + x):
            #     self.table.setRowCount(min(len(v.upQueue),v.maxListCount) + x)
            self.table.setRowCount(x)
            # self.table.setRowCount(len(v.sliceQueue) + x)
            self.table.setItem(0, 0, QTableWidgetItem(v.sliceQueue[0]["fileName"]))
            self.table.setItem(0, 1, QTableWidgetItem(os.path.dirname(v.sliceQueue[0]["path"])))
        ...
        if v.currentHandle != None:
            try:
                ...
                # if (self.table.rowCount()!=min(len(v.upQueue),v.maxListCount) + x):
                self.table.setRowCount(x)
                self.table.setItem(0, 0, QTableWidgetItem(v.currentHandle["fileName"]))
                self.table.setItem(0, 1, QTableWidgetItem(os.path.dirname(v.currentHandle["path"])))
                handle = ""
        ...
        # if (self.table.rowCount()!=min(len(v.upQueue),v.maxListCount) + x):
        self.table.setRowCount(min(len(v.upQueue) + x,v.maxListCount))
        # self.table.setRowCount(min(len(v.upQueue),v.maxListCount) + x)
        for i in range(min(len(v.upQueue) + x,v.maxListCount) -x ):
            try:
                self.table.setItem(i + x, 0, QTableWidgetItem(v.upQueue[i]["fileName"]))
                self.table.setItem(i + x, 1, QTableWidgetItem(os.path.dirname(v.upQueue[i]["path"])))
                self.table.setItem(i + x, 2, QTableWidgetItem(v.upQueue[i]["status"]))
